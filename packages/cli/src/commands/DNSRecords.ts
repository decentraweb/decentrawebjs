...
const FORMATTERS = {
  ...
  'MX': (record: Record.MX) => `${record.type}\t${record.class}\t${record.data.preference} ${record.data.exchange}`,
...
class DNSRecords extends Command {
  ...
  async listRecords(): Promise<void> {
    const lines: string[] = [
      '',
      chalk.bold(`DNS records in ${chalk.cyan(this.name.name)}:`)
    ]
    ...
    choices.push(
      {value: 'exit', title: 'Return', description: "Exit DNS editing"},
    )
    ...
    const {recordType, ttl} = await prompts([
      {
        ...
        choices: [
          ...
          {value: 'AAAA', title: 'AAAA', description: "Used to map domain name to an IPv6 address"},
          {value: 'CNAME', title: 'CNAME', description: "An alias that points to another domain or subdomain"},
          {value: 'MX', title: 'MX', description: "Uses mail servers to map where to deliver email for a domain"},
          {
            value: 'TXT',
            title: 'TXT',
  ...
  async commit() {
    const lines = [
      chalk.bold('Following records will be written to blockchain:')
    ];
    const typeStats = {
    ...
    const {confirmed} = await prompts({
      type: 'confirm',
      name: 'confirmed',
      message: 'Do you want to write data to blockchain? Your wallet will be charged for this transaction.',
      initial: false
    });
    if(!confirmed){
      return this.listRecords();
    ...
    if(!records.length){
      ...
      const needCleanup = Object.keys(typeStats.deleted).some((typeName)=> !typeStats.saved[typeName]);
      if(needCleanup){
        const res = await this.name.clearDNS();
        await waitForTransaction(res, 'Cleaning DNS Zone...')
      }
